...
void accept_request(void *arg)
{
    ...
    //numchars = read(client, buf, sizeof(buf));//获取客户端请求的全部内容
    ...
    //在系统上去查询该文件是否存在,通过文件名filename获取文件信息，并保存在buf所指的结构体stat中, 执行成功则返回0，失败返回-1
    if (stat(path, &st) == -1) 
    ...
    else//文件存在，那去跟常量S_IFMT相与，相与之后的值可以用来判断该文件是什么类型的
    {
        ...
        const char *method, const char *query_string)
{
    ...
    buf[0] = 'A'; buf[1] = '\0';
    //如果 http 请求是 GET 方法的话读取并忽略请求剩下的内容
    if (strcasecmp(method, "GET") == 0)
        while ((numchars > 0) && strcmp("\n", buf))  /* read & discard headers */
            numchars = get_line(client, buf, sizeof(buf));
    //只有 POST 方法才继续读内容
    else if (strcasecmp(method, "POST") == 0) /*POST*/
    {
        ...
        while ((numchars > 0) && strcmp("\n", buf))
        {
            ...
            numchars = get_line(client, buf, sizeof(buf));
            printf("%s", buf);//debug info 客户端的所有body请求数据
        }
        if (content_length == -1) {
            bad_request(client);
            return;
    ...
    //pipe半双工，fd[1]向管道写入数据和fd[0]从管道读取数据只能有一个，亲缘关系进程通信
    //fifo全双工,一端既可以进行读取fd[0]，也可以进行写入fd[1]，可以在任意进程之间建立连接
    ...
    if (pid == 0)  /* child: CGI script */
    {
        ...
        dup2(cgi_input[0], STDIN); //将子进程的输出由标准输入重定向到 cgi_ouput 的管道读端上
        ...
        if (strcasecmp(method, "POST") == 0)
            for (i = 0; i < content_length; i++) {
                recv(client, &c, 1, 0);//获取post数据
                write(cgi_input[1], &c, 1);
            }

        //然后从 cgi_output 管道中读子进程的输出，并发送到客户端去
        while (read(cgi_output[0], &c, 1) > 0)
            send(client, &c, 1, 0);//将cgi脚本执行后的数据发送到客户端
...
int get_line(int sock, char *buf, int size)
{
    ...
    while ((i < size - 1) && (c != '\n'))
    {
        ...
        if (n > 0)
        {
            if (c == '\r')
            {
                //MSG_PEEK设置项不会将buf中的数据移除，再次调用仍然能读到刚刚的数据
...
int startup(u_short *port)
{
    ...
    //设置socketfd可复用，SO_REUSEADDR 仅仅表示可以重用本地本地地址、本地端口
